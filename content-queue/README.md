...
Supabase automatically on a schedule (a few posts per day, spaced apart), so new
...
- **`schedule.json`**, maps every `slug` → its scheduled publish time (ISO UTC).
 Generated by your own cadence (e.g. N/day, spaced a fixed number of minutes
 apart, starting from a chosen date).
